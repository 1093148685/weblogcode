...
<header class="sticky top-0 z-10">
        <nav class="bg-gray-50/80 backdrop-blur-sm border-gray-200 border-b dark:bg-gray-900 dark:border-gray-800">
...
<ul
                        class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-transparent dark:border-gray-700">
                        <li>
                            <a @click="router.push('/')"
                                :class="[currPath == '/' && route.query.view !== 'message-wall' && route.query.view !== 'ai-chat' ? 'text-sky-600 md:border-b-2 md:border-sky-600 dark:text-sky-500 dark:md:border-sky-600' : 'text-gray-900 dark:text-white']"
                        ...
                        <li>
                            <a @click="router.push({ path: '/', query: { view: 'message-wall' } })"
                                :class="[isMessageWallActive ? 'text-sky-600 md:border-b-2 md:border-sky-600 dark:text-sky-500 dark:md:border-sky-600' : 'text-gray-900 dark:text-white']"
                        ...
                        <li>
                            <a @click="router.push({ path: '/', query: { view: 'ai-chat' } })"
                                :class="[isAiChatActive ? 'text-sky-600 md:border-b-2 md:border-sky-600 dark:text-sky-500 dark:md:border-sky-600' : 'text-gray-900 dark:text-white']"
                                class="block py-2 pl-3 pr-4 rounded md:rounded-none hover:bg-gray-100 md:hover:bg-transparent md:hover:text-sky-600 md:p-0 md:dark:hover:text-sky-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700">AI 聊天</a>
                        </li>
                    </ul>
                </div>
            </div>
...
// 留言板是否激活（检查是否在首页且 view=message-wall）
const isMessageWallActive = computed(() => {
    return route.path === '/' && route.query.view === 'message-wall'
})

// AI 聊天是否激活（检查是否在首页且 view=ai-chat）
const isAiChatActive = computed(() => {
    return route.path === '/' && route.query.view === 'ai-chat'
})
...
function renderSearchArticles(data) {
    ...
    getArticleSearchPageList(data).then(res => {
        ...
        if (res.success) {
            ...
            pages.value = Math.ceil((res.data.total || 0) / (res.data.pageSize || 10))
        }
