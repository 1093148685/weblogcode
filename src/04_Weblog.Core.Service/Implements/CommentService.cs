...
public class CommentService : ICommentService
{
    ...
    public async Task<CommentDto> CreateAsync(CreateCommentRequest request)
    {
        ...
        if (initialStatus == 2)
        {
            try
            {
                var notificationSettings = await _blogSettingsService.GetAsync();
                if (notificationSettings.IsEmailNotificationOpen)
                {
    ...
    public async Task<List<CommentDto>> GetPortalListAsync(string? routerUrl)
    {
        var query = _dbContext.CommentDb
            .Where(it => !it.IsDeleted && it.Status == 2); // 仅显示已通过的评论。
